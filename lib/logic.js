...
async function userUsePoints(userusePoints) {
  ...
  if (userusePoints.member.points < userusePoints.points) {
    throw new Error('Insufficient points');
    }
  else{

    //update member points
    userusePoints.member.points = userusePoints.member.points - userusePoints.points;

    let credit=userusePoints.credit;
    ...
    // check if company exists on the network
      let companyRegistry = await getParticipantRegistry('org.pikachu2.biznet.Company');
      companyExists = await companyRegistry.exists(userusePoints.company.id);
      if (companyExists == false) {
      throw new Error('请检查是否选择公司');
      }
      else{userusePoints.company.points = userusePoints.company.points + userusePoints.points;
           await companyRegistry.update(userusePoints.company);
      }
    }

    //对于中等信用等级用户，允许与高、中等商户交易
      ...
      for(let n=0;n<result.length;n++){
        if(userusePoints.company==result[n]){
          userusePoints.company.points = userusePoints.company.points + userusePoints.points;
          await companyRegistry.update(userusePoints.company);
        }
        else{
          throw new Error('对于中等信用等级用户，只允许与高、中等商户交易')
        }
      }
    }
    //对于低等级用户，只能与高商户交易
      ...
      for(let n=0;n<result.length;n++){
        if(userusePoints.company==result[n]){
          ...
          throw new Error('对于低等级用户，只能与高商户交易')
        }
      }
     }
    else{
